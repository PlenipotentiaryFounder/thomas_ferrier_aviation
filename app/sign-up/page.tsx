  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 p-4">
      <div className="w-full max-w-md bg-white/90 dark:bg-slate-900/90 rounded-2xl shadow-2xl p-8 flex flex-col items-center">
        ...
        <p className="text-slate-500 dark:text-slate-300 mb-6 text-center">Create your professional aviation website in minutes.</p>
        <form onSubmit={handleSignUp} className="w-full flex flex-col gap-4">
          ...
          <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-lg shadow transition">Sign Up</button>
          {error && <div className="w-full text-center text-red-600 bg-red-50 border border-red-200 rounded-lg py-2 mt-2">{error}</div>}
          {success && <div className="w-full text-center text-green-700 bg-green-50 border border-green-200 rounded-lg py-2 mt-2">{success}</div>}
        </form>
